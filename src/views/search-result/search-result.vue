          ...
          <div class="title-time_box">
            <div class="time-cell">
              ...
              <span class="item-cell_value">{{
                formatMonthDay(route.query.startDate * 1, "MM.DD")
              }}</span>
            </div>
            <div class="time-cell">
              <span class="time-cell_label">离</span>
              <span class="item-cell_value">{{
                formatMonthDay(route.query.endDate * 1, "MM.DD")
              }}</span>
            </div>
          </div>
...
getSearchKeyWord().then((res) => {
  console.log(res);
  filterAddressInfo.value = res.data.allConditions[0].subGroups.map((item) => ({
  ...
  filterFavorInfo.value = res.data.allConditions[1].items.map((item) => ({
    text: item.label,
    value: item.value,
  }));
  filterFavor.value = filterFavorInfo.value[0]?.value;
  filterInfo.value = res.data.allConditions[2].subGroups.map((item) => ({
    text: item.label,
