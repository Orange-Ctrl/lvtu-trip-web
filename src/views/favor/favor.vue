  ...
  <div class="favor">
    <van-sticky>
      <z-navbar backtext="旅途">
        <template #title>
          <div class="title-box">
            <div
              :class="['title', currentTitleIndex == 0 ? 'title-active' : '']"
              @click="titleChange(0)"
            >
              我的收藏
            </div>
            <div
              :class="['title', currentTitleIndex == 1 ? 'title-active' : '']"
              @click="titleChange(1)"
            >
              浏览历史
            </div>
    ...
    >
      <van-tab v-for="(item, index) in typeList" :key="index" :title="item">
        <div v-if="favorList.length > 0" class="item-box">
          <item
            v-for="(info, subIndex) in favorList"
            :key="subIndex"
            :info="info"
          ></item>
        </div>
    ...
    <template v-else>
      <div v-if="historyList.length > 0" class="item-box">
        <item
          v-for="(info, subIndex) in historyList"
          :key="subIndex"
          :info="info"
        ></item>
      </div>
